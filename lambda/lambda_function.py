...
class GetMostRecentShootingHandler(AbstractRequestHandler):
    """Handler for Skill Launch and GetMostRecentShootingIntent."""

    def can_handle(self, handler_input):
        # type: (HandlerInput) -> bool
        return (is_request_type("LaunchRequest")(handler_input) or
                is_intent_name("GetMostRecentShootingIntent")(handler_input))

    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        logger.info("In GetMostRecentShootingIntent")

        # get localization data
        data = handler_input.attributes_manager.request_attributes["_"]

        shootings_data = shootings.getShootingsData()
        speech = shootings.getRecentSpeech(shootings_data)
        #shooting_date =  (shootings_data[0]["date"])
        #wounded = (shootings_data[0]["wounded"])
        #city = (shootings_data[0]["city"])
        #state_code = (shootings_data[0]["state"])
        #names = (shootings_data[0]["names"])
